                    ...
                    stages {
                        ...
                        stage('Validate Rules') {
                            steps {
                                ...
                                sh 'FILES=$(ls *_rules.yaml 2>/dev/null || true); if [ -z "$FILES" ]; then echo "No files found in $(pwd)"; ls -la || true; exit 1; fi; echo "Found rule files: $FILES"; for file in $FILES; do docker run --rm -u $(id -u):$(id -g) --entrypoint /bin/promtool -v "$(pwd)":/workspace -w /workspace prom/prometheus:latest check rules $file; done'
                            }
                        }
