                    ...
                    stages {
                        ...
                        stage('Validate Rules') {
                            steps {
                                ...
                                sh 'FILES=$(find rules -name "*_rules.yaml"); if [ -z "$FILES" ]; then echo "No files found"; exit 1; fi; docker run --rm --entrypoint /bin/promtool -v "$(pwd)":/workspace -w /workspace prom/prometheus:latest check rules $FILES'
                            }
                        }
