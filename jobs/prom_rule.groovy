                    ...
                    stages {
                        ...
                        stage('Validate Rules') {
                            steps {
                                ...
                                sh 'FILES=$(ls rules/*_rules.yaml 2>/dev/null || true); if [ -z "$FILES" ]; then echo "No files found in $(pwd)/rules"; ls -la rules || true; exit 1; fi; echo "Found rule files: $FILES"; docker run --rm --entrypoint /bin/promtool -v "$(pwd)":/workspace -w /workspace prom/prometheus:latest check $FILES'
                            }
                        }
