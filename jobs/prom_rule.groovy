                    ...
                    stages {
                        ...
                        stage('Validate Rules') {
                            steps {
                                ...
                                sh 'FILES=$(git ls-files "rules/prometheus_rules.yaml"); if [ -z "$FILES" ]; then FILES=$(ls rules/prometheus_rules.yaml 2>/dev/null || true); fi; if [ -z "$FILES" ]; then echo "No files found"; exit 1; fi; docker run --rm --entrypoint /bin/promtool -v "$(pwd)":/workspace -w /workspace prom/prometheus:latest check $FILES'
                            }
                        }
